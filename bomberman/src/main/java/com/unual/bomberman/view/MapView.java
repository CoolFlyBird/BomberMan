        ...
        for (y = 0; y < gameConfig.HEIGHT_SIZE; y++) {
            for (x = 0; x < gameConfig.WIDTH_SIZE; x++) {
                if (gameConfig.X_OFFSET + (x + 1) * gameConfig.PER_WIDTH < 0) {
                    ...
                    continue;
                } else if (gameConfig.SCREEN_WIDTH != 0 && gameConfig.X_OFFSET + x * gameConfig.PER_WIDTH > gameConfig.SCREEN_WIDTH) {
                    continue;
                } else if (gameConfig.SCREEN_HEIGHT != 0 && gameConfig.Y_OFFSET + y * gameConfig.PER_HEIGHT > gameConfig.SCREEN_HEIGHT) {
                    continue;
                }
                switch (gameConfig.getMapInfo().getInfo()[x][y]) {
                    case GameConfig.MAP_TYPE_BRICK:
                        canvas.drawBitmap(gameConfig.getBrick(), gameConfig.X_OFFSET + gameConfig.X_DIRECTION_OFFSET + x * gameConfig.PER_WIDTH, gameConfig.Y_OFFSET + gameConfig.Y_DIRECTION_OFFSET + y * gameConfig.PER_HEIGHT, null);
                        break;
                    case GameConfig.MAP_TYPE_WALL:
                        canvas.drawBitmap(gameConfig.getWall(), gameConfig.X_OFFSET + gameConfig.X_DIRECTION_OFFSET + x * gameConfig.PER_WIDTH, gameConfig.Y_OFFSET + gameConfig.Y_DIRECTION_OFFSET + y * gameConfig.PER_HEIGHT, null);
                        break;
                    case GameConfig.MAP_TYPE_TEMP:
                    case GameConfig.MAP_TYPE_FIRE:
                    case GameConfig.MAP_TYPE_BACKGROUND:
                        canvas.drawBitmap(gameConfig.getBackground(), gameConfig.X_OFFSET + gameConfig.X_DIRECTION_OFFSET + x * gameConfig.PER_WIDTH, gameConfig.Y_OFFSET + gameConfig.Y_DIRECTION_OFFSET + y * gameConfig.PER_HEIGHT, null);
                        break;
                }
            }
        }
